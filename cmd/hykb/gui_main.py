...
from bothykb_main import run_hykb
...
DEFAULT_CONFIG = {
    ...
    'api': {
        ...
        'download_url': 'http://fees.5156xz.com/api/pick/callback/appInfo'
    },
    'sftp': {
        'remote_dir': '/home/down/emulator',
        'remote_icon_dir': '/home/apkicon/'
    }
}
...
class BotHYKBGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("好游快爆 自动化采集助手 v1.0")
        ...
        self.log_dir = os.path.join(self.base_dir, 'logs', 'hykb')
    ...
    def setup_styles(self):
        ...
        style.configure("TLabelframe", background="#1e1e1e", foreground="#4CAF50")
        style.configure("TLabelframe.Label", background="#1e1e1e", foreground="#4CAF50", font=("Microsoft YaHei UI", 10, "bold"))
    ...
    def save_config_file(self):
        try:
            ...
            if 'api' not in new_config: new_config['api'] = {}
            new_config['api']['share_url'] = self.vars['share_url'].get()
            new_config['api']['download_url'] = self.vars['download_url'].get()
            
            if 'sftp' not in new_config: new_config['sftp'] = {}
            new_config['sftp']['remote_dir'] = self.vars['remote_dir'].get()
            new_config['sftp']['remote_icon_dir'] = self.vars['remote_icon_dir'].get()
    ...
    def setup_monitor_tab(self, parent):
        ...
        tk.Label(title_frame, text="🎮 好游快爆 自动化采集助手", font=("Microsoft YaHei UI", 20, "bold"), fg="#4CAF50", bg="#1e1e1e").pack()
        ...
        self.start_btn = tk.Button(btn_frame, text="▶ 启动", font=("Microsoft YaHei UI", 11, "bold"), bg="#4CAF50", fg="white", 
    ...
    def setup_settings_tab(self, parent):
        ...
        self.vars['download_url'] = tk.StringVar(value=api_conf.get('download_url', ''))
        self.create_labeled_entry(group_api, "采集上报 URL:", self.vars['share_url'], width=50)
        self.create_labeled_entry(group_api, "下载上报 URL:", self.vars['download_url'], width=50)

        # SFTP
        group_sftp = ttk.LabelFrame(scrollable_frame, text="SFTP 远程路径", padding=10)
        group_sftp.pack(fill=tk.X, pady=10)
        sftp_conf = self.current_config.get('sftp', {})
        self.vars['remote_dir'] = tk.StringVar(value=sftp_conf.get('remote_dir', ''))
        self.vars['remote_icon_dir'] = tk.StringVar(value=sftp_conf.get('remote_icon_dir', ''))
        self.create_labeled_entry(group_sftp, "APK 远程路径:", self.vars['remote_dir'], width=50)
        self.create_labeled_entry(group_sftp, "Icon 远程路径:", self.vars['remote_icon_dir'], width=50)
    ...
    def run_worker(self):
        try:
            run_hykb(stop_func=lambda: not self.is_running)
        except Exception as e:
